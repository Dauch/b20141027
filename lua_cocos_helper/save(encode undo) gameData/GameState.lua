...
function GameState.init(eventListener_, stateFilename_, secretKey_)
    if type(eventListener_) ~= "function" then
        error("GameState.init() - invalid eventListener")
        return false
    end
...
function GameState.load()
    ...
    if not io.exists(filename) then
        print(string.format("GameState.load() - file \"%s\" not found", filename))
        return eventListener({name = "load", errorCode = GameState.ERROR_STATE_FILE_NOT_FOUND})
    end

    local contents = io.readfile(filename)
    print(string.format("GameState.load() - get values from \"%s\"", filename))
    ...
    if secretKey and isEncodedContents_(contents) then
        ...
        if type(values) ~= "table" then
            error("GameState.load() - invalid data")
...
function GameState.save(newValues)
    ...
    if type(values) ~= "table" then
        error("GameState.save() - listener return invalid data")
        return false
    end
    ...
    print(string.format("GameState.save() - update file \"%s\"", filename))
    return ret
end

function GameState.getGameStatePath()
    return string.gsub(cc.FileUtils:getInstance():getWritablePath(), "[\\\\/]+$", "") .. "/" .. stateFilename
end
